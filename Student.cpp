...
using namespace std;
  ...
  void Student::StudentID(string val) {
	...
	int Student::validEmail() {
		int valTemp = 0;
		int x = this->_emailAddr.find("@");
		int y = this->_emailAddr.rfind(".");
		int z = this->_emailAddr.find(" ");
		if (z != -1) { valTemp = 1; } // Contains a Space
		else if (y - x < -1 && y != -1) { valTemp = 2; } // . preceeds @
		else if (y - x == 0) { valTemp = 4; } // both @ and . are mising
		else if (x == -1) { valTemp = 3; } // @ symbol is missing
		else if (y == -1) { valTemp = 5; } // The . is missing
		return valTemp;
	...
	void Student::printInvalidEmail() {
		char buffer[200];
		string result="";
		int ev=this->validEmail();
		if (ev>0)
		{
			int x = sprintf_s(buffer, "%s|%s|%s|%s - Email Is Invalid: %s\n",  this->_studentID.c_str(), this->_firstName.c_str(), this->_lastName.c_str(), this->_emailAddr.c_str(), this->EvilEmails[ev].c_str());
			result = buffer;
		}
		cout << (ev==0? "" : result);
	}
	...
	void Student::Print() {
		printf("%s\tFirst Name : %s\tLast Name : %s\tAge : %s\tdaysInCourse : {%s, %s, %s}\tDegree Program : %s\n", _studentID.c_str(), _firstName.c_str(), _lastName.c_str(), to_string(_age).c_str(), to_string(_duration[0]).c_str(), to_string(_duration[1]).c_str(), to_string(_duration[2]).c_str(), Degree2String[(int)_degreePr].c_str());
